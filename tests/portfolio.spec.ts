...
test("light is default even on dark OS; night mode persists and contact is current", async ({page})=>{
 ...
 await expect(page.locator('#experience')).toContainText('Jan 2026 – June 2026');
 await expect(page.locator('#experience')).toContainText('XFactr Softwares Private Limited');
 await expect(page.locator('#about')).toContainText('9.67');
 await expect(page.locator('a[href="tel:+917730886127"]')).toBeVisible();
 await page.setViewportSize({width:390,height:844});
 await expect(page.getByRole('button',{name:'Night mode',exact:true})).toBeVisible();
